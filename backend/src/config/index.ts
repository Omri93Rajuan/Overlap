...
export const config = {
    ...
    users: {
        uri: env.get('USERS_SERVICE_URI').asString(),
        baseRoute: env.get('USERS_BASE_ROUTE').default('/api/users').asString(),
    },
    systems: {
        uri: env.get('SYSTEMS_SERVICE_URI').asString(),
    ...
    authentication: {
        ...
        callbackURL: env.get('CALLBACK_URL').asString(),
        shragaURL: env.get('SHRAGA_URL').asString(),
        ...
        token: env.get('ACCESS_TOKEN_NAME').asString(),
